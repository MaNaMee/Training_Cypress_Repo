...
Cypress.Commands.add('AE_CreateUser', () => {
    // let generatedSampleData = generateTestData()
  //cy.writeFile('cypress/fixtures/UserCredentials.json', generatedSampleData);
  // cy.get('.shop-menu > .nav > :nth-child(4)').should('contain',' Cart').should('not.be.disabled').click()
  ...
  cy.fixture('RegisteredUser.json').then((UserInformation) => {
    ...
    cy.get('[data-qa="days"]').select(UserInformation.days)
...
Cypress.Commands.add('Verify_Delivery_Invoice', (type, RegisteredUser) => {
    ...
    // cy.get(`#address_${type} > :nth-child(3)`).should('contain', RegisteredUser.company);
